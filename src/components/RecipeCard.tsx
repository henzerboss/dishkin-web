...
export function RecipeCard({ recipe, locale }: Props) {
  ...
  return (
    <Link href={`/${locale}/recipes/${recipe.slug}`} className="card group overflow-hidden transition duration-200 hover:-translate-y-1 hover:shadow-2xl">
      ...
      <div className="p-5">
        <div className="mb-3 flex flex-wrap gap-2">
          {recipe.categories.slice(0, 3).map((c) => <span key={c.id} className="badge py-1 text-xs">{c.name}</span>)}
        </div>
